...
don't have to construct these tasks ourselves. Gym is an open-source toolkit
...
different amounts of observability and uncertainty. Therefore, the first two
environments (Copy-v0 and Taxi-v2) are fully observable with deterministic
action-selection and the other two (Blackjack-v0 and FrozenLake-v0) are not
...
where the task is to copy symbols from an input to an output tape. Taxi-v2
introduces a bit more complexity. It tasks agents with picking up and dropping
off a passenger at a designated location. At all times, the location of the
agent, passenger, and the destination are available. The goal in Blackjack-v0
is to find a winning card playing strategy against a computer dealer. We
selected this environment because +we can become millionaires+ of the partial
observability and uncertainty of the environment. In FrozenLake, the agent is
tasked to move across a grid of ice with holes in it. Because of the slippery
nature of the ice, the agent won't always move in the intended direction;
tension rises! As the agent is not given any information in advance of, or
during the game, this environment requires the agent to form a strategy solely
on previous paths and rewards.
